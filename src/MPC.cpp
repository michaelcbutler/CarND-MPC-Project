...
// TODO: Set the timestep length and duration - See MPC.h
...
// const double Lf = 2.67; moved to MPC.h
...
const double wt_cte = 1000.0;
const double wt_epsi = 4000.0;
const double wt_v = 1.0;

const double wt_delta = 1000;
const double wt_a = 50.0;

const double wt_delta_dot = 5000.0; // d/dt(delta)
const double wt_a_dot = 100.0; // d/dt(a)
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < MPC::N - 1; t++) {
      ...
      fg[2 + psi_start + t] = psi1 - (psi0 - v0 * delta0 / MPC::Lf * MPC::dt); // delta sign issue correction
      fg[2 + v_start + t] = v1 - (v0 + a0 * MPC::dt);
      fg[2 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * MPC::dt));
      fg[2 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0 * delta0 / MPC::Lf * MPC::dt); // delta sign issue correction
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //std::cout << "Cost " << cost << " solution status = " << (ok?"OK":"not OK") << std::endl;
